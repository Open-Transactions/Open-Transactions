	...
	else
	{
		...
		if (theNym.Loadx509CertAndPrivateKey(false))
		{
			const time64_t	CURRENT_TIME	= OTTimeGetCurrentTime(),
                VALID_TO = OTTimeAddTimeInterval(CURRENT_TIME, OTTimeGetSecondsFromTime(OT_TIME_SIX_MONTHS_IN_SECONDS)),	// Tokens generated by this mint are valid from today until 6 months from today
                MINT_EXPIRATION = OTTimeAddTimeInterval(CURRENT_TIME, OTTimeGetSecondsFromTime(OT_TIME_THREE_MONTHS_IN_SECONDS));	// The mint itself will expire in 3 months from today, and be replaced with a new one.
